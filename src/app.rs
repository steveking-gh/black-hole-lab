...
mod tests {
    ...
    #[test]
    fn test_the_strength_fade_is_calibrated_to_the_field_it_is_drawn_over() {
        // `Theme::STRENGTH_FADE_DECADES` is the one free number in the strength display: it says
        // how much of the alpha range a decade of fluence is worth. The physics fixes everything
        // else - what the strength is, and that the alpha is linear in its logarithm - but not
        // this, and a number chosen away from the picture is how a channel ends up carrying a
        // measurement nobody can see. At four decades the whole of Alice's field came out between
        // alpha 100 and 170, a factor of 1.7 that reads as no variation at all.
        //
        // So it is measured here, on the run the app opens on, at both ends: the top of the range
        // has to be reached, the bottom has to be used, and the cut at the faint end must not be
        // eating the picture. What is measured is the strength of each drawn segment rather than
        // the strokes themselves; `spatial_canvas` has the test that the strokes carry it.
        let mut app = SpacetimeApp::default();
        for _ in 0..400 {
            app.step_forward(0.05);
        }
        // Alice's transmission, which is the ordinary case: an emitter outside the horizon whose
        // fronts spread and weaken. Bob's is the other one and is looked at below.
        let alphas = |field: &crate::physics::wavefront::SignalField| -> Vec<u8> {
            let mut out = Vec::new();
            for pulse in &field.pulses {
                ...
                for i in 0..n {
                    ...
                    if pulse.rays[i].alive() && pulse.rays[j].alive() {
                        let strength = pulse.segment_strength(&app.metric, i, j);
                        out.push(Theme::strength_alpha(strength, Theme::SHIFT_ALPHA));
                    }
                }
            }
            out
        ...
        let alice = alphas(&app.signal);
        assert!(alice.len() > 1000, "the default run has a field to measure: {}", alice.len());
        let full = Theme::SHIFT_ALPHA;
        let cut = alice.iter().filter(|a| **a == 0).count() as f64 / alice.len() as f64;
        let faint =
            alice.iter().filter(|a| **a < full / 2).count() as f64 / alice.len() as f64;
        let brightest = *alice.iter().max().expect("a non-empty field");
        println!(
            "Alice's field after 20 M: {} segments drawn, brightest at alpha {brightest} of \
             {full}, {:.0}% of them below half of that, {:.1}% cut as too faint",
            alice.len(),
            100.0 * faint,
            100.0 * cut
        );
        assert_eq!(brightest, full, "a fresh front still reaches full opacity");
        assert!(faint > 0.2, "and a fifth of the field is drawn at under half of it: {faint}");
        assert!(cut < 0.02, "while the cut takes almost none of it: {cut}");

        // Bob's, which falls: his old fronts are deep inside and genuinely faint, so the cut does
        // real work there, and at the other end his frozen arcs sit at the top of the range. Both
        // ends of the ramp are in use in one picture.
        let bob = alphas(&app.bob_signal);
        let bob_cut = bob.iter().filter(|a| **a == 0).count() as f64 / bob.len() as f64;
        let bob_full = bob.iter().filter(|a| **a == full).count() as f64 / bob.len() as f64;
        println!(
            "Bob's: {} segments, {:.0}% cut as too faint and {:.0}% at full opacity",
            bob.len(),
            100.0 * bob_cut,
            100.0 * bob_full
        );
        assert!(bob_full > 0.1, "the light piling onto r- is drawn solid: {bob_full}");
    }
