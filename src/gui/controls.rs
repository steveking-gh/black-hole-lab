...
const FRONT_ARCS_TIP: &str = "Whether the app draws the pieces of a wavefront between neighbouring rays. Ticked, each piece is the curve linear in (r, ϕ) from one ray to the next, cut into steps of at most 0.05 rad, with each step put through the embedding x + iy = (r + ia)e^{iϕ} — so a piece joining two rays sitting on r₋ comes out as an arc of the r₋ circle, and a piece joining two rays a quarter of a turn apart comes out going round. Behind every drawn piece the app lays a fade, in that piece's own colour draining to nothing, on the side the light at that piece has come from — so a paused frame says which way each front travels. Each fade reaches back only as far as the gap to the front behind it allows, up to 30 pixels, so the fades of one transmission never stack: crowded fronts each keep a soft trailing edge a few pixels deep, and a deep zoom or a low front count opens the fade out to its full length. The fade follows the coordinate velocity of the rays themselves rather than the shape of the drawn curve, which is the honest choice where a front shears or folds, and the frozen family on r₋ carries no fade at all, since that light glides along the front rather than across the front. Unticked, nothing appears between the rays: the front shows as the calculated points themselves, one dot per ray in the same gain colour the arc would have carried, and the frozen family keeps the heavier beads.
