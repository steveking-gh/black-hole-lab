...
pub struct AppControls {
    ...
    /// Whether a segment of a wavefront between two neighbouring rays is drawn as the curve linear
    /// in (r, phi) between them, or as the straight chord between their two screen positions.
    ///
    /// A drawing choice and nothing else: the reception test interpolates in (r, phi) along that
    /// same segment either way, so this cannot move an arrival or change a measured shift. Default
    /// on, because on is the drawing that matches what is being detected.
...
const FRONT_ARCS_TIP: &str = "What the piece of a wavefront between two neighbouring rays is drawn as. Ticked, it is the curve linear in (r, ϕ) from one ray to the other, cut into pieces of at most 0.05 rad and each piece put through the embedding x + iy = (r + ia)e^{iϕ} — so a segment joining two rays sitting on r₋ is drawn as an arc of the r₋ circle, and one joining two rays a quarter of a turn apart is drawn going round. Unticked, it is the straight chord between the two rays' positions on screen. Nothing physical turns on it. The reception test interpolates in (r, ϕ) along exactly that same segment whichever way it is drawn, so an arrival happens at the same event, at the same measured shift, in both settings; what the tick buys is that the front you are looking at is the same curve the detector is testing, and what unticking buys is a faster and plainer drawing — the raw polygon on the rays themselves — which is worth being able to see. It is worth knowing what the chords get wrong before trusting them in the deep interior. Inside r₋ the annulus is thin (at a = 0.90 the embedding puts r₋ at ρ = 1.06 against the ring at ρ = 0.90) and neighbouring rays wind at wildly different rates, dϕ/dt running from about −5 per M near the ring to +0.8 for one settling onto r₋, so a pair of neighbours ends up most of a radian apart. The chord between two rays that far apart cuts straight across the annulus and through the disk inside the ring: it draws spikes into the singularity along paths no ray of the front ever took. The arcs are on by default for that reason.";
